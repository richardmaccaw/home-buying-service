...
export class ScrapingService {
  ...
  async scrapeRightmovePage(url: string): Promise<string> {
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, ScrapingService.DELAY_MS),
      );
  ...
  private extractBedrooms($: cheerio.CheerioAPI): string {
    ...
    for (const selector of bedroomSelectors) {
      ...
      if (bedroomElement.length > 0) {
        ...
        // Look for patterns like "5 bedrooms", "5 bed", "BEDROOMS: 5", etc.
        const bedroomMatch = bedroomText.match(
          /(\d+)\s*(?:bedrooms?|BEDROOMS?)/i,
        );
        if (bedroomMatch) {
          return `Bedrooms: ${bedroomMatch[1]}\n`;
        }
      }
    }

    // Fallback: Look for any text containing bedroom numbers
    const bodyText = $("body").text();
    const bedroomMatch = bodyText.match(/(\d+)\s*(?:bedrooms?|BEDROOMS?)/i);
    if (bedroomMatch) {
      return `Bedrooms: ${bedroomMatch[1]}\n`;
    }
  ...
  private extractPriceReductions($: cheerio.CheerioAPI): string {
    ...
    for (const selector of dateSelectors) {
      ...
      if (dateElement.length > 0) {
        ...
        if (
          dateText &&
          (dateText.toLowerCase().includes("reduced") ||
            dateText.toLowerCase().includes("added"))
        ) {
  ...
  private isValidImage(
    ...
    baseUrl: string,
    seenUrls: Set<string>,
    seenBaseUrls: Set<string>,
  ): boolean {
    const isRightmoveMedia =
      url.includes("media.rightmove") || url.includes("rightmove-static");
  ...
  private isAcceptableQuality(url: string): boolean {
    const isPropertyImage =
      /_IMG_\d{2}_\d{4}\.(jpe?g|png|webp)$/i.test(url) ||
  ...
  private isSystemImage(url: string): boolean {
    ...
    return (
      systemImagePatterns.some((pattern) => url.includes(pattern)) ||
      /\/[A-Z]{2,}_/.test(url) ||
