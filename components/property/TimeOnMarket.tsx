...
export function TimeOnMarket() {
  ...
  const getMarketTimeCategory = (days: number) => {
    ...
    if (days <= 7)
      return { label: "Just listed", color: "bg-green-500", position: 10 };
    if (days <= 14)
      return { label: "Recent", color: "bg-lime-500", position: 20 };
    if (days <= 30)
      return { label: "Recent", color: "bg-yellow-500", position: 35 };
    if (days <= 60)
      return { label: "Available", color: "bg-orange-500", position: 55 };
    if (days <= 90)
      return { label: "Some time", color: "bg-orange-600", position: 75 };
    if (days <= 180)
      return { label: "Long time", color: "bg-red-500", position: 85 };
  ...
  const getTimeDescription = (days: number) => {
    ...
    if (days <= 30)
      return "This is a recently listed property. Standard negotiation rules apply.";
    if (days <= 60)
      return "This property has been available for a moderate time. There may be some room for negotiation.";
    if (days <= 90)
      return "This property has been on the market for some time. There may be room for negotiation.";
    return "Properties that have been on the market a long time might have more wiggle room for negotiation.";
  };
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1 py-4">
            ...
            >
              <div
                className={`${category.color} text-white px-3 py-2 rounded-lg font-semibold text-sm shadow-lg`}
              >
                {timeLabel}
              </div>
            </div>
